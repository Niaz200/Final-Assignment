							...
							<div class="jbs-head-bodys-top my-5">
								...
								<div class="jbs-roots-y1 flex-column justify-content-start align-items-start">
									<div class="jbs-roots-y1-last">
										...
										<div class="jbs-urt mb-2"><span class="label text-light primary-2-bg">{{$singlejobs->job_type}}</span></div>
										<div class="jbs-title-iop mb-1"><h2 class="m-0 fs-2 text-light">{{$singlejobs->job_title}}</h2></div>
										<div class="jbs-locat-oiu text-sm-muted text-light d-flex align-items-center">
											<span><i class="fa-solid fa-location-dot opacity-75 me-1"></i>{{$singlejobs->location}}</span>
									...
									<div class="jbs-roots-y6 py-3">
										...
										<form action="{{ route('stoteapply') }}" method="POST">
                                            ...
                                            @csrf
                                            <input type="hidden" name="id" value="{{ $singlejobs->id }}">
                                            <input type="hidden" name="employer_id" value="{{ $singlejobs->user_id }}">
                                            <input type="hidden" name="job_title" value="{{ $singlejobs->job_title}}">
                                            <input type="hidden" name="job_type" value="{{ $singlejobs->job_type }}">
                                            <input type="hidden" name="location" value="{{ $singlejobs->location }}">
                                            <input type="hidden" name="compa_name" value="{{ $singlejobs->company_name }}">

									<div class="jbs-roots-y6 py-3">
										<button class="btn btn-primary fw-medium px-lg-5 px-4 me-3" type="submit" data-bs-toggle="modal" data-bs-target="#applyjob">Apply Job</button>
							...
							<div class="explot-info-details d-inline-flex flex-wrap">
								...
								<div class="single-explot d-flex align-items-center me-md-5 me-3 my-2">
									...
									<div class="single-explot-last ps-2">
										...
										<p class="text-light fw-bold fs-6 m-0">{{$singlejobs->location}}</p>
									</div>
								</div>
								<div class="single-explot d-flex align-items-center">
									...
									<div class="single-explot-last ps-2">
										...
										<p class="text-light fw-bold fs-6 m-0">{{$singlejobs->min_salary}}-{{$singlejobs->max_salary}} Tk</p>
									</div>
								</div>
							</div>
									...
									<div class="jbs-content-body mb-4">
										...
										<div class="jbs-content mb-4">
											...
											<ul class="simple-list">
													...
													@php
														$reqrs = $singlejobs->responsibilities;
														$respons= explode("\n", $reqrs);
													@endphp
										...
										<div class="jbs-content">
											...
											$kils=(implode(',', $singlejobs->skills));
